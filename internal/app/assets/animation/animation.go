...
// smoothqueue: even start spacing + concurrency cap (respect roblox limits
// don't exceed) actual values come from the tuning package the desktop app
// scales them at launch based on the user's detected pc specs
...
// when retry-after is missing on 429 (rare); server hint via ideratelimiterror otherwise
...
// grantholderpermission mirrors the sound pipeline's behaviour: for every
// configured holder issue a use permission on the freshly uploaded animation
// best-effort failures get logged but don't block the upload flow
func grantHolderPermission(c *roblox.Client, assetID int64, holderUserIDs []string, logErr func(args ...any)) {
	...
	for try := 0; try < 2; try++ {
		...
		if err != nil {
			if logErr != nil {
				logErr(fmt.Sprintf("permission grant prep failed for animation %d: %v", assetID, err))
			}
			return
		}
		_, err = handler()
		...
		if logErr != nil {
			logErr(fmt.Sprintf("permission grant failed for animation %d: %v", assetID, err))
		}
		return
	}
}
...
func Reupload(ctx *context.Context, r *request.Request) {
	...
	uploadQueue := taskqueue.NewSmoothQueue[int64](tuning.AnimationMaxConcurrent(), tuning.AnimationStartsPerMinute())
	// aimd: configured rate is the *starting* point successful uploads slowly
	// pull the gap toward mingap (≈25x configured rate as a ceiling); a 429
	// multiplies the gap by 17 toward maxgap (≈4x slower than configured)
	// this auto-tunes to whatever roblox is actually willing to do right now
	// instead of dying on a fixed value that may be too aggressive today
	{
		base := time.Minute / time.Duration(tuning.AnimationStartsPerMinute())
		uploadQueue.EnableAIMD(base*2/5, base*4, 20)
	}
	...
	newUploadError := func(m string, assetInfo *develop.AssetInfo, err any) {
		...
		if assetInfo != nil {
			// notify the http hub so the desktop app can log "no <oldid> (reason)"
			// inline with successes instead of only seeing it at the end of the job
	...
	uploadAsset := func(wg *sync.WaitGroup, assetInfo *develop.AssetInfo, location string) {
		...
		res := <-uploadQueue.QueueTask(func() (int64, error) {
			id, err := retry.Do(
				...
				func(try int) (int64, error) {
					...
					case ide.UploadAnimationErrors.ErrNotLoggedIn:
						// in the electron-spawned process there is no terminal attached so
						// getnewcookie would detect non-interactive and cancel the whole job
						// skip it here and let the retry loop try again instead
					...
					default:
						if errors.Is(err, ide.ErrRateLimited) && try < animationUploadRetryTries {
							// permanent rate adjustment: widen the aimd gap so every
							// in-flight goroutine paces itself slower from now on
							// without this we'd keep hammering at the same pre-429 rate
							// after retry-after elapses and immediately bounce again
							uploadQueue.RecordRateLimit()
			...
			// feed the aimd limiter so a long run of clean uploads gradually
			// pulls the gap toward mingap ie we earn back speed after 429s
			uploadQueue.RecordSuccess()
		...
		// grant the configured holders use permission on the new animation id
		// pool mode pulls holders from the pool single-account mode pulls from
		// the request when permissionsharing is enabled
		var holderIDs []string
		if ctx.Pool != nil {
			holderIDs = ctx.Pool.HolderIDs()
		}
		...
		if len(holderIDs) > 0 {
			go grantHolderPermission(client, newID, holderIDs, ctx.Logger.Error)
		}
	}
